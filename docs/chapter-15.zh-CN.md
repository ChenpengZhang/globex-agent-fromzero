...
为了继续学习持久化以及后续的长期记忆，本章前端作为一个完整模块直接实现。它始终只是
...
页面；下一次请求会在后端创建全新的 Agent session。buyer ID 保持不变，使第 18
章能够把跨 session 的持久化偏好关联到同一买家。
...
完成这一边界后，第 16 章可以在稳定的 Application 边界之后继续加入文件持久化、
session 恢复和对话历史；第 17 章接入关系型数据库，长期买家偏好顺延到第 18 章。
